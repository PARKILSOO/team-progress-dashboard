...
신생 스타트업의 팀장은 팀이 10명으로 성장하면서 기존의 엑셀 기반 진행상황 관리가 더 이상 버티기 어려워진다. 매번 팀원들을 일일이 물어봐야 하고, 정보는 항상 오래되고, 신속한 의사결정이 불가능해진다. 이 팀 진행상황 대시보드는 '팀의 상태를 한눈에, 그리고 1초 내에' 파악할 수 있도록 설계되었다. 그 핵심은 단순한 실시간 업데이트가 아니라, **AI가 개별 데이터를 자동으로 해석해서 팀장이 30초 안에 핵심만 이해할 수 있게 하는 것**이다. 이를 통해 팀장은 의사결정에 더 빠르고 정확히 대응할 수 있고, 팀원들은 반복적인 보고 마찰에서 벗어난다.

---

### 네러티브 구성 요소

- **대상**: 신생 스타트업의 팀장, 10명 규모의 빠르게 성장하는 팀 [명시]
- **진짜 문제**: 팀 규모 증가로 인한 수동 관리의 한계 - 정보 지연, 신속한 의사결정 불가, 팀원 피로도 증가 [명시]
- **맥락·타이밍**: 팀이 10명으로 성장하는 시점에 기존 엑셀 기반 관리가 버티기 어려워짐 [명시]
- **핵심 관점·베팅**: AI가 자동으로 개별 데이터를 해석해주면 팀장의 의사결정 속도가 극적으로 빨라진다 [명시]
- **해결 방향**: 실시간 통합 대시보드 + AI 자동 요약으로 팀 상태의 '의미'를 팀장에게 전달 [명시]
- **성공의 모습**: 팀장이 30초 안에 팀 상태와 주요 블로커를 파악, 팀원은 간단한 입력으로 보고 완료 [명시]
